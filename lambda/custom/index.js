...
// const APP_ID = REPLACE_WITH_YOUR_OWN_GENERATED_KEY

function SpeakSyntax(detail, month)
{
    let sentence = detail.Day + " " + detail.Date + " " + month + " 2018 <break time='0.5s'/> <say-as interpret-as='interjection'>" + detail.Event + "</say-as> as a " + detail.reason+ ". <break time='1s'/>";
    return sentence;
}
...
const handlers = {
    ...
    'MonthIntent': function () {
        var speechOutput = "Here are the details. <break time='0.5s'/>. ";
        let month = this.event.request.intent.slots.Month.value;
        let details = data[month];
        for (let i in details) {
            speechOutput += SpeakSyntax(details[i], month);
        }
        const reprompt = HELP_REPROMPT;
        this.response.speak(speechOutput).listen(reprompt);
        this.emit(':responseReady');
